...
router.put('/unlike/:id', auth, async (req, res) => {
  try {
    ...
    const i = post.likes.map(like => like.user.toString()).indexOf(req.user.id)
    if (i === -1) {
...
router.delete('/comment/:id/:comment_id', auth, async (req, res) => {
  try {
    ...
    const i = post.comments.indexOf(comment);
    if (i === -1) {
      return res.status(400).json({ msg: 'No such comment ID' });
    }
